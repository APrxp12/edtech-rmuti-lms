...
function LessonResultContent() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-16 print:p-0 print:space-y-4 print:max-w-full">
      ...
      <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-around text-center text-xs print:hidden shadow-xs">
      ...
      <div className="p-5 sm:p-6 bg-gradient-to-br from-blue-50/90 via-indigo-50/40 to-sky-50/60 rounded-3xl border border-blue-100 shadow-sm space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-blue-100/90 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold text-blue-800 bg-white px-3 py-1 rounded-full border border-blue-200 shadow-2xs font-mono">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-white text-blue-700 flex items-center justify-center font-bold border border-blue-100 shadow-2xs shrink-0">
              <User className="w-5 h-5" />
            </div>
            <div>
              <span className="text-[10px] text-slate-500 block">ผู้เข้าศึกษา / ผู้ทำแบบทดสอบ:</span>
          ...
          <div className="flex items-center gap-3 sm:justify-end">
            <div className="w-10 h-10 rounded-2xl bg-white text-indigo-700 flex items-center justify-center font-bold border border-blue-100 shadow-2xs shrink-0">
              <GraduationCap className="w-5 h-5" />
            </div>
            <div>
              <span className="text-[10px] text-slate-500 block">รหัสนักศึกษา:</span>
              <span className="text-sm sm:text-base font-mono font-bold text-slate-900 leading-snug">
                {studentIdDisplay}
              </span>
            </div>
          </div>
        ...
        <div className="pt-2 border-t border-blue-100/80 text-xs text-blue-950 font-semibold flex items-center justify-between">
          <span>บทเรียน: {lesson.title.startsWith('บทที่') ? lesson.title : `บทที่ ${lesson.sortOrder} ${lesson.title}`}</span>
          <span className="font-mono text-blue-700 text-[11px] bg-white px-2 py-0.5 rounded border border-blue-200">
            {lesson.code}
          </span>
        </div>
      </div>
      ...
      {isPassed ? (
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-emerald-200 shadow-lg space-y-6">
          <div className="text-center space-y-3">
            <div className="w-20 h-20 rounded-3xl bg-emerald-100 text-emerald-600 mx-auto flex items-center justify-center shadow-inner print:w-14 print:h-14">
              <Trophy className="w-10 h-10 text-emerald-600 print:w-7 print:h-7" />
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center space-y-1">
              ...
              <div className="text-xl font-black text-slate-800">{passScorePercent}%</div>
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden">
            <div className="p-3.5 bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-800 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500 font-normal">บันทึกอัตโนมัติ</span>
            </div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-600">
                ...
                {progress?.preTestScore && (
                  <tr>
                    ...
                    <td className="py-2.5 px-4 text-slate-500">ก่อนเริ่มเรียน</td>
                ...
                {postAttempts.length > 0 ? (
                  postAttempts.map((att, idx) => {
                    ...
                    return (
                      <tr key={idx} className={isBest ? 'bg-emerald-50/50 font-semibold' : ''}>
                        ...
                        <td className={`py-2.5 px-4 font-mono ${isBest ? 'text-emerald-700 font-bold' : ''}`}>
                        ...
                        <td className="py-2.5 px-4">
                          ...
                          {isBest && <span className="ml-1 text-[10px] text-amber-600 font-bold">(นับคะแนน)</span>}
                        </td>
                      </tr>
                    );
                  })
                  ...
                  <tr className="bg-emerald-50/40 font-semibold">
                    <td className="py-2.5 px-4 font-mono">1</td>
                    <td className="py-2.5 px-4">แบบทดสอบหลังเรียน (Post-test)</td>
                    <td className="py-2.5 px-4 text-slate-500">เพิ่งเสร็จสิ้น</td>
                    <td className="py-2.5 px-4 font-mono text-emerald-700">{displayScore}%</td>
                    <td className="py-2.5 px-4 text-emerald-700">✓ ผ่านเกณฑ์ (นับคะแนน)</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 pt-2 print:hidden">
            <Link
              ...
              className="w-full sm:w-1/4 py-2.5 px-4 rounded-xl border border-slate-200 text-slate-700 text-xs font-bold hover:bg-slate-50 transition text-center"
            >
              ทบทวนบทเรียน
            </Link>
            ...
            <Link
              href="/dashboard"
              className="w-full sm:w-1/4 py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition text-center shadow-md shadow-blue-200"
            >
              กลับแดชบอร์ด
            </Link>
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="w-full sm:w-1/2 py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition flex items-center justify-center gap-2 shadow-md shadow-emerald-200 cursor-pointer"
            >
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-amber-200 shadow-lg space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            ...
            <div className="p-4 bg-red-50 rounded-2xl border border-red-100 text-center space-y-1">
              ...
              <div className="text-xl font-black text-red-700">{passScorePercent}%</div>
              <span className="text-[9px] text-red-600 font-bold">ต้องได้ {passScorePercent}% ขึ้นไป</span>
            </div>
            ...
            <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 text-center space-y-1">
              ...
              <div className="text-xl font-black text-blue-800">{remainingAttempts} ครั้ง</div>
              <span className="text-[9px] text-blue-600">จากทั้งหมด {maxAttempts} ครั้ง</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 pt-2 print:hidden">
            {remainingAttempts > 0 ? (
              <Link
                ...
                className="w-full sm:w-1/2 py-3 px-6 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold shadow-md shadow-amber-200 transition flex items-center justify-center gap-2 cursor-pointer"
              >
              ...
              <button
                disabled
                className="w-full sm:w-1/2 py-3 px-6 rounded-xl bg-slate-200 text-slate-400 text-xs font-bold cursor-not-allowed flex items-center justify-center gap-2"
              >
